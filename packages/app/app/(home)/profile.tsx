...
export default function Profile() {
  ...
  const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: theme.background, padding: 20, paddingTop: 60, gap: 16 },
  ...
  return (
    <View style={styles.container}>
      ...
      {user?.imageUrl && <Image source={{ uri: user.imageUrl }} style={{ width: 48, height: 48, borderRadius: 24 }} />}
      <Text style={{ color: theme.foreground }}>{user?.fullName ?? user?.id}</Text>
      <TouchableOpacity style={styles.button} onPress={presentUserProfile}>
        <Text style={styles.buttonText}>Manage Profile</Text>
      </TouchableOpacity>
      <TouchableOpacity style={[styles.button, { backgroundColor: theme.primary }]} onPress={() => signOut()}>
