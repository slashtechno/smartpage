...
function TabButton({ isFocused, children, ...props }: { isFocused?: boolean; children?: React.ReactNode; [key: string]: any }) {
  return (
    <Pressable className="px-4 py-3" {...props}>
      <Text className={isFocused ? "text-primary font-semibold" : "text-foreground"}>
        {children}
      </Text>
    </Pressable>
  );
